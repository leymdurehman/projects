...
SELECT name, continent, governmentform, headofstate FROM country WHERE continent = 'Europe';
...
-- parenthesis important here
SELECT name, region FROM country WHERE (continent = 'North America' OR continent = 'South America') AND region != 'Caribbean';
...
SELECT name, (gnp/population) AS percapitaGNP FROM country WHERE continent = 'Europe';


-- 22. The number of years since independence for all countries that have a year of independence (192 rows)

SELECT name, (2021 - indepyear) AS yearssinceindep FROM country WHERE indepyear IS NOT NULL;