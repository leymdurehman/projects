...
SELECT film.title, category.name        // I did the C.R.U.D. HW first which affected my amt of return rows
FROM category
...
SELECT film.title, category.name, release_year
FROM category
JOIN film_category ON film_category.category_id = category.category_id
JOIN film ON film.film_id = film_category.film_id
WHERE release_year = 2006 AND name = 'Sci-Fi';
...
SELECT film.title, category.name, COUNT(rental.customer_id)
FROM rental
JOIN inventory ON rental.inventory_id = inventory.inventory_id
JOIN film ON inventory.film_id = film.film_id
JOIN film_category ON film_category.film_id = film.film_id
...
-- (#1 should be “GINA DEGENERES” with 753 rentals and #10 should be “SEAN GUINESS” with 599 rentals)
 -- actor iD to film id to inventory -filmid to inventory-id to rental inventory id -call rental.customerid
 
SELECT actor.first_name, actor.last_name, actor.actor_id, count(rental.customer_id)
FROM actor
JOIN film_actor ON film_actor.actor_id = actor.actor_id
JOIN film ON film_actor.film_id = film.film_id
