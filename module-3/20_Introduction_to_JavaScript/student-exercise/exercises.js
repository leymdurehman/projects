...
*/
		...
		function lastDigit(x, y){
			if ((String(y).substr(-1)) == (String(x).substr(-1))) { 
			return true;
		}   else 
			return false;                                           
...
*/
		...
		function frontAgain(string){
			if (string.length == 2) {
				return true;
			} else if (string.substring(0,1) == string.substring((string.length-2),(string.length-1))){
				return true;
			} else
				return false;
		}
...
*/
		...
		function cigarParty(cigars, isWeekend){
			...
			// could be 2 params or arr[] w 2 data types
